...
每轮 propose 都有一个 `decisionId`，无论是否配置了决策存储；结果、慢思考请求的 `subject` 和决策记录都带着它。决策记录的 `provider` 取决策自身的 `provider` 字段（路由器、规则、预算守卫可以写明到底是谁做的决定），没有才用配置的决策器名称；Jev 适配器总是写上模型名。轮次失败或请求慢思考时，`DecisionSubject` 说明原因（没有候选、决策器请示、在哪个阶段以什么码失败）。错误码只信任 HubError 与字符串 code：浏览器标准 DOMException 的数字旧码没有意义，超时记为 `timeout`，其余记为 `decision-unavailable`。
