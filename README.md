...
已实现：类型化契约；依赖驱动的插件激活与回滚；作用域能力注册；能力停用与在途排空；有限候选构造；直接 Jev HTTP 适配；状态/策略过期拒绝；默认预览；单次提案消费；进程内幂等与资源预留；未知结果核对；人工请求收件箱；内存事件记录。

尚未实现：PostgreSQL/outbox、进程重启恢复、分布式资源锁、网络认证服务、签名执行许可、插件沙箱/市场、MCP、C# SDK、真实机器人适配器、任意目标规划、完整自主调度循环。详见 [实现边界](docs/architecture.md) 和 [路线图](docs/roadmap.md)。
...
| [架构决策](docs/adr/0001-foundation.md) | 技术栈与首版取舍 |
