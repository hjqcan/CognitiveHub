...
Use stable operation IDs and canonical resource names. Propagate idempotency keys to the real executor. A timeout can conceal a successful external effect: reconcile it; do not blindly retry. The memory journal does not survive restart and does not coordinate multiple processes. Do not run production irreversible actions until persistent recovery and executor-side deduplication are implemented and tested.
